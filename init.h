...
void vect_trans(double vect1[4], double J[4][4]) {
    double vect2[4];
    for (int i = 0; i < 4; i++) {
        vect2[i] = 0.0;
        for (int j = 0; j < 4; j++) {
...
double ut_cal(double g[4][4], double u1, double u2, double u3) {
    double AA = g[0][0];
...
void convert_ui(int i, int j, int k) {
    ...
    for(int index = 1; index < 4; index++)
        betaArr[index - 1] = gcon[0][index]*alpha*alpha;
    double Gamma = alpha*uu[0];
    prim[i][j][k][U0] = uu[0];
    prim[i][j][k][U1] = uu[1] + Gamma * betaArr[0] / alpha;
    prim[i][j][k][U2] = uu[2] + Gamma * betaArr[1] / alpha;
    prim[i][j][k][U3] = uu[3] + Gamma * betaArr[2] / alpha;
}
            ...
            for (int k = 0; k < N3; k++) {
                ...
                ***************************************************************************************************/
                ...
                if (r >= rin && lnh >= 0.) {
                    ...
                    prim[i][j][k][UU] = u * (1. + 4e-2 * (rancval - 0.5));
